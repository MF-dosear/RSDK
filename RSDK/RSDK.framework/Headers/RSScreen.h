//
//  RSScreen.h
//  YXGAMESDK
//
//  Created by Paul on 2021/8/15.
//  Copyright © 2020 Carp. All rights reserved.
//

#import <Foundation/Foundation.h>
#import <UIKit/UIKit.h>

NS_ASSUME_NONNULL_BEGIN

typedef NS_ENUM(NSUInteger, YXScreenOrientation) {
    YXScreenOrientationNone = 0,    // 不确定
    YXScreenOrientationVertical,    // 竖屏
    YXScreenOrientationHorizontal   // 横屏
};


typedef NS_ENUM(NSUInteger, YXScreenMode) {
    iPhone_5S_SE                 = 568,
    iPhone_6_6S_7_8              = 667,
    iPhone_6P_6SP_7P_8P          = 736,
    iPhone_12mini                = 780,
    iPhone_X_XS_11Pro            = 812,
    iPhone_12__12Pro             = 844,
    iPhone_XR_XSMax_11_11ProMax  = 896,
    iPhone_12ProMax              = 926,
};

@interface RSScreen : NSObject

// 返回指定屏幕的宽度
@property (nonatomic,assign) CGFloat width;

// 宽度相对于 6s 375的比例
@property (nonatomic,assign) CGFloat ratio;

// NavBar的高度
@property (nonatomic,assign) CGFloat navBarH;

// NavBar X增加的高度
@property (nonatomic,assign) CGFloat navBarAddH;

// nav Status的高度
@property (nonatomic,assign) CGFloat navStatusH;

// X 左边偏移位置
@property (nonatomic,assign) CGFloat mas_left;

@property (nonatomic,assign) CGFloat landscapeRight_MasLeft;

// tabBar的高度
@property (nonatomic,assign) CGFloat tabBarH;

// 刘海的高度
@property (nonatomic,assign) CGFloat black_h;

// tabBar X增加的高度
@property (nonatomic,assign) CGFloat tabBarAddH;

// 横屏 竖屏
@property (nonatomic,assign) YXScreenOrientation orientation;

// 横屏 竖屏 比例
@property (nonatomic,assign) CGFloat orientationRatio;

@property (nonatomic,assign) YXScreenMode mode;

+ (instancetype)shared;

@end


NS_ASSUME_NONNULL_END
